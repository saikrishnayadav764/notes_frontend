...
const EditBox = (props) => {
  ...
  const editTask = (e) => {
    ...
    fetch(`https://oscowbackend-production.up.railway.app/api/todos/${editData.todoId}`, {
      method: "PUT",
      body: JSON.stringify({
      ...
      .then((json) => {
        let updatedTasks = tasks.map((task) => {
          if (task.todoId === editData.todoId) {
            return editData;
          }
          return task;
        });
  ...
  const formatDate = (dateString) => {
    ...
    const month = String(date.getMonth() + 1).padStart(2, '0'); // Adding 1 since getMonth() is 0-indexed
          ...
          <div className="task-inputDateTime">
            <div className="task-date">
              ...
              <input
                type="Date"
                id="task-date-input"
                value={formatDate(editData.date)}
                onChange={(e) => {
